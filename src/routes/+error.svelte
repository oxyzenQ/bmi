...
<script lang="ts">
	...
	let props = $props<{ status: number; message: string }>();

	let statusCode = $derived(props.status);
	...
	let description = $derived(
		statusCode === 404 ? t('error.not_found_desc') : props.message || t('error.internal_desc')
	);
