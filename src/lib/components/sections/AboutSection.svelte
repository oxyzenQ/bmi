				...
				{#if $pwaInstallState.checked}
					<section class="pwa-install-card" aria-labelledby="pwa-install-title">
						<div
							...
							class:pwa-installed={$pwaInstallState.isInstalled}
							aria-hidden="true"
						>
						...
						<div class="pwa-install-copy">
							...
							<p>
								{$pwaInstallState.isInstalled
									? t('pwa.installed_description')
									: t('pwa.not_installed_description')}
							</p>
						</div>
						<div class="pwa-install-actions">
							...
							{:else}
								<button
									...
									disabled={$pwaUpdateState.checking || updateCheckingOverlay}
								>
									<PackageCheck size={18} aria-hidden="true" />
									<span
										>{$pwaUpdateState.checking || updateCheckingOverlay
											? t('pwa.update_checking')
											: t('pwa.check_update')}</span
									>
								</button>
							{/if}
						</div>
					...
					<div class="about-card-content">
						...
						<div class="app-info">
							<p class="info-row">
								...
								<strong>{t('about.version')}:</strong>{getStellarVersionLabel()}
								<span class="commit-id">({gitCommitId})</span>
							</p>
							{#if gitBranch !== 'main'}
								<p class="info-row">
							...
							<p class="info-row">
								...
								<strong>{t('about.type_apps')}:</strong><span class="text-gradient-elegant"
									>{t('about.pwa_type')}</span
								>
							</p>
							<p class="info-row">
								<Database class="Database" />
