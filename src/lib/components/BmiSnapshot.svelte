...
<div class="gauge-container bmi-snapshot">
	...
	<div class="snapshot-cards">
		<!-- Current BMI Card -->
		<div
			class="snapshot-card current {currentBmi !== null
				? getStatusBg(currentBmi)
				: 'status-unknown'}"
		>
			...
			<div
				class="card-value"
				style={currentBmi !== null ? `color: ${getStatusColor(currentBmi)}` : ''}
			>
		...
		<div class="snapshot-card target {currentBmi !== null ? '' : 'status-unknown'}">
			...
			<div class="card-category">
				{currentBmi !== null ? t('snapshot.optimal') : t('snapshot.na')}
			</div>
		</div>
	</div>
				...
				{#if bestBmi !== null && Math.abs(stats.diffFromBest) > 0.1}
					<div class="insight-item comparison">
						{#if stats.isImprovement}
							...
							<span
								>{t('snapshot.improvement', { n: Math.abs(stats.diffFromBest).toFixed(2) })}</span
							>
						{:else}
...
<style>
	...
	.progress-fill.progress-lose {
		background: linear-gradient(
			90deg,
			var(--cat-red-90) 0%,
			var(--cat-yellow-40) 50%,
			var(--cat-green-90) 100%
		);
	}
