...
<script lang="ts">
	...
	onMount(() => {
		...
		(window as unknown as Record<string, { toggle: () => void }>).__bmi_debug_panel = {
			toggle: togglePanel
		};
				...
				{#if activeTab === 'logs'}
					...
					<div class="dp-controls">
						<select
							...
							value={logFilter}
							onchange={(e) => {
								logFilter = (e.target as HTMLSelectElement).value as LogLevel | 'ALL';
								refreshLogs();
							}}
						>
						...
						<label class="dp-auto-scroll">
							<input
								type="checkbox"
								checked={autoScroll}
								onchange={() => (autoScroll = !autoScroll)}
							/>
							...
							{#each logs.slice().reverse() as entry (entry.timestamp + entry.seq)}
								<div class="dp-log-entry" class:dp-log-error={entry.level === 'ERROR'}>
									<div class="dp-log-header">
										<span class="dp-log-level" style="color:{LOG_LEVEL_COLORS[entry.level]}"
											>{entry.level}</span
										>
									...
									{#if entry.error}
										<div class="dp-log-error-detail">
											<span class="dp-log-error-name">{entry.error.name}:</span>
											{entry.error.message}
										</div>
									{/if}
									{#if entry.data}
					...
					{#if systemInfo}
						<div class="dp-system-grid">
							...
							<div class="dp-system-row">
								...
								<span
									class="dp-value"
									class:ok={systemInfo.localStorageAvailable}
									class:err={!systemInfo.localStorageAvailable}
								>
							...
							<div class="dp-system-row">
								...
								<span
									class="dp-value"
									class:ok={systemInfo.indexedDbAvailable}
									class:err={!systemInfo.indexedDbAvailable}
								>
						...
						{:else}
							{#each storageEntries as entry (entry.key)}
								<div class="dp-storage-item">
									<button
										class="dp-storage-key"
										onclick={() => toggleKey(entry.key)}
										aria-expanded={expandedKeys.has(entry.key)}
									>
										<ChevronRight
											size={12}
											style={expandedKeys.has(entry.key) ? 'transform: rotate(90deg)' : ''}
										/>
