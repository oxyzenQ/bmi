...
<script lang="ts">
	...
	function estimateBodyFat(bmiVal: number, ageVal: number, isMale: boolean): number {
		...
		return 1.2 * bmiVal + 0.23 * ageVal - 10.8 * sexFactor - 5.4;
	}
	...
	function getCategory(
		...
		if (isMale) {
			if (bf < 6)
				return { label: t('fat.essential'), color: COLORS.BLUE, desc: t('fat.essential_desc') };
			if (bf < 14)
				return { label: t('fat.athletic'), color: COLORS.GREEN, desc: t('fat.athletic_desc') };
			if (bf < 18)
				return { label: t('fat.fitness'), color: COLORS.GREEN, desc: t('fat.fitness_desc') };
			if (bf < 25)
				return { label: t('fat.average'), color: COLORS.YELLOW, desc: t('fat.average_desc') };
			return {
				label: t('fat.above_average'),
				color: COLORS.RED,
				desc: t('fat.above_average_desc')
			};
		} else {
			if (bf < 14)
				return { label: t('fat.essential'), color: COLORS.BLUE, desc: t('fat.essential_desc') };
		...
		<div class="bf-ranges">
			...
			{#if sex === 'male'}
				<div class="range-row">
					...
					<div class="range-bar">
						<div class="range-fill" style="width: 24%; background: var(--cat-blue-40);"></div>
					</div>
				...
				<div class="range-row">
					...
					<div class="range-bar">
						<div class="range-fill" style="width: 32%; background: var(--cat-green-40);"></div>
					</div>
				...
				<div class="range-row">
					...
					<div class="range-bar">
						<div class="range-fill" style="width: 36%; background: var(--cat-green-40);"></div>
					</div>
				...
				<div class="range-row">
					...
					<div class="range-bar">
						<div class="range-fill" style="width: 56%; background: var(--cat-amber-40);"></div>
					</div>
				...
				<div class="range-row">
					...
					<div class="range-bar">
						<div class="range-fill" style="width: 75%; background: var(--cat-red-40);"></div>
					</div>
			...
			{:else}
				<div class="range-row">
					<span class="range-label">{t('fat.range_essential')}</span>
					<div class="range-bar">
						<div class="range-fill" style="width: 28%; background: var(--cat-blue-40);"></div>
					</div>
				...
				<div class="range-row">
					<span class="range-label">{t('fat.range_athletic')}</span>
					<div class="range-bar">
						<div class="range-fill" style="width: 42%; background: var(--cat-green-40);"></div>
					</div>
