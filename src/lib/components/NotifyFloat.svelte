...
<script lang="ts">
	...
	const Icon = $derived(
		type === 'success' ? CheckCircle : type === 'warn' ? ShieldAlert : type === 'error' ? X : Trash2
	);
...
>
	...
	{#if type === 'delete' || type === 'warn'}
		<div class="notify-btn-group">
			<button class="notify-btn btn-cancel" onclick={handleCancel}>
				{t('notify.cancel')}
			</button>
			<button class="notify-btn {buttonClass}" onclick={handleContinue}>
				{buttonText}
			</button>
		</div>
	{:else if type === 'error'}
...
<style>
	...
	.btn-success {
		background: linear-gradient(135deg, rgba(5, 46, 22, 0.8), rgba(22, 101, 52, 0.6));
		color: var(--stellar-white);
	...
	.btn-success:hover {
		...
		background: linear-gradient(135deg, rgba(5, 46, 22, 0.9), rgba(22, 101, 52, 0.7));
	...
	.btn-warn {
		background: linear-gradient(135deg, rgba(69, 26, 3, 0.8), rgba(146, 64, 14, 0.6));
		color: var(--stellar-white);
	...
	.btn-warn:hover {
		transform: translateY(-2px) scale(1.02);
		background: linear-gradient(135deg, rgba(69, 26, 3, 0.9), rgba(146, 64, 14, 0.7));
