...
<script lang="ts">
	...
	import { GAUGE, getPerformanceTier, prefersReducedMotion } from '$lib/utils/animation-config';
	import {
		CATEGORY_COLORS,
		COLORS,
		BMI_THRESHOLDS,
	...
	let { bmi = 0, category = null, ultraSmooth = false }: Props = $props();
	...
	let categoryScale = $derived([
		{
			key: 'Underweight',
			label: t('gauge.underweight'),
			color: categoryColors['Underweight'],
			min: 0,
			max: 18.5
		},
		{
			key: 'Normal Weight',
			label: t('gauge.normal'),
			color: categoryColors['Normal Weight'],
			min: 18.5,
			max: 25.0
		},
		{
			key: 'Overweight',
			label: t('gauge.overweight'),
			color: categoryColors['Overweight'],
			min: 25.0,
			max: 30.0
		},
	...
	function getScaleClass(key: string): string {
		switch (key) {
			case 'Underweight':
				return 'sc-underweight';
			case 'Normal Weight':
				return 'sc-normal';
			case 'Overweight':
				return 'sc-overweight';
			case 'Obese':
				return 'sc-obese';
			default:
				return '';
	...
	$effect(() => {
		...
		bmiTweenDuration = reducedMotion
			...
			: ultraEnabled
				? perfTier === 'high'
					? GAUGE.TWEEN_DURATION_HIGH
					: GAUGE.TWEEN_DURATION_MEDIUM
				: perfTier === 'low'
					? GAUGE.TWEEN_DURATION_LOW
					: GAUGE.TWEEN_DURATION_DEFAULT;

		strokeDuration = reducedMotion
			? '0ms'
			: ultraEnabled
				? perfTier === 'high'
					? GAUGE.STROKE_DUR_HIGH
					: GAUGE.STROKE_DUR_MEDIUM
		...
		strokeDurationFill = reducedMotion
			...
			: ultraEnabled
				? perfTier === 'high'
					? GAUGE.STROKE_FILL_DUR_HIGH
					: GAUGE.STROKE_FILL_DUR_MEDIUM
				: GAUGE.STROKE_FILL_DUR_DEFAULT;

		strokeDelayFill = ultraEnabled
			? GAUGE.STROKE_DELAY_FILL_ENHANCED
			: GAUGE.STROKE_DELAY_FILL_DEFAULT;
	...
	let categoryDisplayText = $derived(
		appliedCategory ? t(CATEGORY_I18N_KEYS[appliedCategory] ?? 'gauge.normal') : t('gauge.na')
	);
	...
	<div class="gauge-wrapper">
		...
		>
			...
			<circle
				...
				class="gauge-progress {appliedBmi > 0 && usePulse ? 'pulsing' : ''} {isFilling
					? 'filling'
					: ''}"
			/>
			<!-- center text -->
		...
		{#if appliedBmi <= 0}
			<div class="gauge-empty-cta">
				...
				<button
					...
					class="gauge-cta-btn"
					onclick={() => {
						if (browser) window.location.hash = '#calculator';
					}}
				>
					{t('gauge.calculate')}
				</button>
