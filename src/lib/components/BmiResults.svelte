...
<script lang="ts">
	...
	let primeLabel = $derived.by(() => {
		...
		if (bmiPrime < 1.0) return t('results.prime_normal');
		if (bmiPrime <= 1.2) return t('results.prime_above');
	...
	let idealMinDisplay = $derived(
		idealMin !== null
			? parseFloat((unitSystem === 'imperial' ? idealMin * KG_TO_LBS : idealMin).toFixed(1))
			: null
	);
	let idealMaxDisplay = $derived(
		idealMax !== null
	...
	let weightDelta = $derived.by(() => {
		...
		if (currentKg < idealMin)
			return {
				amount: parseFloat((idealMin - currentKg).toFixed(1)),
				direction: 'below' as const,
				unit: 'kg'
			};
		if (currentKg > idealMax)
			return {
				amount: parseFloat((currentKg - idealMax).toFixed(1)),
		...
		{#if hasResults}
				...
				<div class="stat-block ideal-block">
					...
					{#if idealMinDisplay !== null && idealMaxDisplay !== null}
						...
						{#if deltaDisplay && deltaDisplay.direction !== 'within'}
							<p
								class="stat-desc delta-desc"
								class:delta-above={deltaDisplay.direction === 'above'}
								class:delta-below={deltaDisplay.direction === 'below'}
							>
								{deltaDisplay.direction === 'above' ? '+' : '−'}{deltaDisplay.amount}
								{deltaDisplay.unit}
								{t('results.from_range')}
							</p>
			...
			<div class="action-buttons-row">
				<button
					...
					class="action-btn"
					onclick={handleShare}
					aria-label={t('results.share_aria')}
				>
					<Share2 size={16} />
				...
				<button
					...
					class="action-btn"
					onclick={handleCopy}
					aria-label={t('results.copy_aria')}
				>
					<Copy size={16} />
				...
				<button
					...
					aria-label={t('results.download_aria')}
				>
					<ImageDown size={16} />
