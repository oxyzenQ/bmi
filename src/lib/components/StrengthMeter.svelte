...
<script lang="ts">
	...
	let strengthLabel = $derived(
		strengthLevel === 'weak'
			? t('crypto.strength_weak')
			: strengthLevel === 'fair'
				? t('crypto.strength_fair')
				: strengthLevel === 'medium'
					? t('crypto.strength_medium')
					: strengthLevel === 'strong'
						? t('crypto.strength_strong')
...
<style>
	...
	.strength-fill {
		...
		border-radius: 2px;
		transition:
			width var(--dur-content) ease,
			background var(--dur-content) ease;
	}
