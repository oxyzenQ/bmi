...
async function loadNonEnglishLocale(code: Locale): Promise<void> {
	...
	try {
		...
		switch (code) {
			case 'id':
				mod = await import('./locales/id');
				break;
			case 'zh':
				mod = await import('./locales/zh');
				break;
			case 'ja':
				mod = await import('./locales/ja');
				break;
			default:
		...
		warnDev(
			'i18n',
			'loadNonEnglishLocale',
			`Failed to load locale: ${code}, falling back to English`,
			err
		);
