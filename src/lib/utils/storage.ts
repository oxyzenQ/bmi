...
import { dbGetAll, dbMetaGet, dbMetaSet, dbRemove, dbSet, isIndexedDbAvailable } from './db';
...
export async function initStorage(): Promise<void> {
	...
	try {
		...
		for (const [key, value] of cache) {
			if (value !== null) {
				try {
					...
					/* quota — warnDev intentionally skipped for perf */
				}
			}
		}
		...
		warnDev(
			'storage',
			'initStorage',
			'IndexedDB init failed, falling back to localStorage-only mode',
			err
		);
		_initialized = true;
	}
...
async function validateAndRepairCache(): Promise<void> {
	...
	for (const key of jsonKeys) {
		...
		try {
			...
			warnDev(
				...
				`Corrupted value detected for key: ${key}, attempting repair`,
				err
			);
				...
				try {
					...
					dbSet(key, lsValue).catch((dbErr) => {
						warnDev(
							'storage',
							'validateAndRepairCache',
							`Failed to repair IndexedDB for key: ${key}`,
							dbErr
						);
					});
					repaired = true;
			...
			dbRemove(key).catch((dbErr) => {
				warnDev(
					'storage',
					'validateAndRepairCache',
					`Failed to remove corrupted key: ${key} from IndexedDB`,
					dbErr
				);
			});
			repaired = true;
	...
	if (repaired) {
		warnDev(
			'storage',
			'validateAndRepairCache',
			'Storage corruption was detected and repaired. Some data may have been lost.'
		);
	}
}
...
async function migrateFromLocalStorage(): Promise<void> {
	...
	if (failedKeys.length > 0) {
		...
		warnDev(
			...
			`Migration incomplete — ${failedKeys.length} keys failed, will retry next load`
		);
		return;
	}
