...
async function deriveKeyArgon2(passphrase: string, salt: Uint8Array): Promise<CryptoKey> {
	...
	const hashBytes = argon2id(new TextEncoder().encode(passphrase), salt, ARGON2_OPTIONS);

	return crypto.subtle.importKey('raw', toAB(hashBytes), { name: 'AES-GCM', length: 256 }, false, [
		'encrypt',
		'decrypt'
	]);
}
...
export async function verifyChecksum(data: string, checksum: string): Promise<boolean> {
	...
	try {
		...
		warnDev(
			'crypto',
			'verifyChecksum',
			'Checksum comparison failed — possibly corrupted base64',
			err
		);
		return false;
	}
	...
	try {
		...
		try {
			...
			warnDev(
				'crypto',
				'decrypt',
				'Encrypted payload contains invalid base64 — data may be corrupted',
				err
			);
			return null;
		}
...
export async function enableEncryption(passphrase: string): Promise<boolean> {
	...
	try {
		...
		warnDev(
			'crypto',
			'enableEncryption',
			'Failed to enable encryption (verifier creation or DB write)',
			err
		);
		return false;
	}
...
export async function getEncryptionStatus(): Promise<EncryptionStatus> {
	...
	try {
		...
		warnDev(
			'crypto',
			'getEncryptionStatus',
			'Failed to read encryption status from IndexedDB',
			err
		);
		return { enabled: false, hasPassphrase: false };
	}
