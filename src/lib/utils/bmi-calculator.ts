 ...
 */
export function validateInputs(heightCm: number, weightKg: number): BmiError | null {
	...
	if (heightCm > INPUT_LIMITS.HEIGHT_MAX_CM) {
		return {
			...
			message: `Height must be ≤ ${INPUT_LIMITS.HEIGHT_MAX_CM} cm.`
		};
	}
	if (weightKg > INPUT_LIMITS.WEIGHT_MAX_KG) {
		return {
			...
			message: `Weight must be ≤ ${INPUT_LIMITS.WEIGHT_MAX_KG} kg.`
		};
	}
	return null;
}
